...
!
      ...
      real(r8), dimension(800) :: Rval ! Modified PWA 18/09/2018: Increased dimension from 200 to 800 to allow for high complexity

      character (len=40 ) :: KeyWord
      character (len=256) :: line
      character (len=256), dimension(800) :: Cval ! Modified PWA 18/09/2018: Increased dimension from 200 to 800 to allow for high complexity
...
! read into FMODEL(ng)%f(tile)%model from the .yaml file when roms_fabm_init
...
! Option 2: Extract their values from FMODEL(ng)%f(tile)%model - SEE BELOW commented out
! Option 3: Do not create biological parameters as internal ROMS variables (CHOSEN 21/07/2015)


!!!!PWA Inserted 21/07/2015
!      property => FMODEL(ng)%f(tile)%model%root%parameters%first
...
#ifdef DEBUG
      write(*,*) "Inputing parameters from rfabm.in in rfabm_inp.h"
#endif
...
!!!End insertion PWA 08/03/2017
            ...
            CASE ('TNUDG')
              ...
              END DO
#ifdef DEBUG
              write(*,*) "Done input of AK and nudging parameters"
#endif
            CASE ('LBC(isTvar)')
...
#ifdef DEBUG
              write(*,*) "Done input of BGC bc parameters"
#endif
     ...
     &                                'idTvar(idbio(', itrc, '))'
              ...
              END DO
#ifdef DEBUG
              write(*,*) "Done input of biological tracer Hout options"
#endif
            CASE ('Hout(idTsur)')
              Npts=load_l(Nval, Cval, NBT*Ngrids, Ltrc)
              DO ng=1,Ngrids
