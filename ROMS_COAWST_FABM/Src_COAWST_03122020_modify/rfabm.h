...
!************************************************** Hernan G. Arango ***
!  Copyright (c) 2002-2020 The ROMS/TOMS Group         Phil Wallhead   !
!                                                   Andre Staalstrom   !
...
!!!Inserted PWA 08/03/2017
      USE mod_forces
#if defined FABM_AICE && defined ICE_MODEL
      USE mod_ice
#endif
      USE fabm
      USE fabm_config
...
!
#include "tile.h"
      IF (MOD(iic(ng)-1,nFABM(ng)).eq.0) THEN
...
!!!PWA Inserted 08/03/2017
      ...
      USE fabm_config
!!!End insertion PWA 08/03/2017
!!!PWA Inserted 31/07/2019 from latest Arango code
!
      USE dateclock_mod, ONLY : caldate
!
!!!End insert PWA 31/07/2019
!
!  Imported variable declarations.
!
      integer, intent(in) :: ng, tile
...
# endif
# ifdef FABM_TSS_ONLINE
      real(r8), intent(inout) :: tss(LBi:,LBj:,:)
# endif
...
# endif
# ifdef FABM_TSS_ONLINE
      real(r8), intent(inout) :: tss(LBi:UBi,LBj:UBj,UBk)
# endif
...
!  Local variable declarations.
!
      integer :: ibio, itrc, ivar, i, j, k, ks, nx, ny, listp
...
!-----------------------------------------------------------------------
     ...
     &           'aice',ai(Istr:Iend,Jstr:Jend,listp))
#endif
